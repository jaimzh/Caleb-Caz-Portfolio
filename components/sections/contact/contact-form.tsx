...
export function ContactForm() {
  return (
    <form 
      className="caleb-card space-y-6"
    ...
    >
      ...
      <div className="space-y-2 relative">
        <label className="text-xs font-bold uppercase tracking-widest text-text-muted">Project Type</label>
        <div className="relative group/select">
          <select className="w-full bg-transparent border-b border-border py-2 px-0 focus:outline-none focus:border-text transition-colors cursor-pointer appearance-none [&>option]:bg-bg [&>option]:text-text">
      ...
      >
        Send Message
        <Send size={18} className="translate-x-0 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform ml-2" />
      </Button>
    </form>
  );
}
