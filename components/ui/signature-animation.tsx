...
export const SignatureStroke = ({ className = "text-black" }) => {
  // We use variants so the parent controls the "start" signal
  const containerVariants = {
    hidden: {},
    visible: {
      transition: {
        staggerChildren: 0.2, // Adjust this to tighten/loosen the sequence
      },
    },
  };
  ...
  return (
    <svg
      ...
      className={className}
      fill="none" // Ensure fill is none so strokes are visible
    ...
    >
      <motion.g
        ...
        transform="matrix(0.587261, 0.00125205, -0.00107854, 0.505877, 67.5205, 7.93525)"
      >
        <g transform="matrix(1.70282, 0, 0, 1.97676, -397.187, -215.749)">
          
          {/* 1. The Main Initial Scribble (Shortest Duration) */}
          <g transform="matrix(1, 0, 0, 1, 0, -1608.05)">
            <motion.path
              ...
              variants={{
                hidden: { pathLength: 0 },
                visible: { pathLength: 1, transition: { duration: 0.9, ease: "easeOut" } }
              }}
            />
          </g>

          {/* 2. The Loop/Complex Detail (Middle Duration) */}
          <g transform="matrix(1, 0, 0, 1, -25.6352, -1608.05)">
            <motion.path
              ...
              variants={{
                hidden: { pathLength: 0 },
                visible: { pathLength: 1, transition: { duration: 1.7, ease: [0.69, 0, 1, 1] } }
              }}
            />
          </g>

          {/* 3. The Long Underline (Last to start, longest duration) */}
          <g transform="matrix(0.999889, 0.0149147, -0.0149147, 0.999889, 241.368, 100.647)">
            <motion.path
              ...
              variants={{
                hidden: { pathLength: 0 },
                visible: { pathLength: 1, transition: { duration: 2.3, ease: "easeInOut" } }
              }}
            />
          </g>
        </g>
